  ...
  `}function W(e){const t=q.querySelector(".order-button-modal");t&&t.addEventListener("click",()=>{const r=q.querySelector('input[name="color"]:checked'),s=r?r.value:e.color[0];M(),d.style.display="flex",document.body.style.overflow="hidden",i.dataset.modelId=e._id,i.dataset.color=s})}S==null||S.addEventListener("click",A);d==null||d.addEventListener("click",e=>{e.target===d&&A()});function A(){d.style.display="none",document.body.style.overflow=""}i==null||i.addEventListener("submit",async e=>{var _,T,B;e.preventDefault();const t=i.querySelector("#orderName").value.trim(),r=i.querySelector("#orderPhone").value.replace(/\D/g,""),s=i.querySelector("#orderComment").value.trim(),o=i.dataset.modelId,n=i.dataset.color;if(!t||!r||!o||!n){a("Будь ласка, заповніть усі обов'язкові поля!");return}if(s&&s.length<5){a("Коментар повинен бути щонайменше 5 символів!");return}const c={name:t,phone:r,modelId:o,color:n,comment:s};console.log("Payload для отправки на сервер:",c);try{const m=await L.post("https://furniture-store-v2.b.goit.study/api/orders",c,{headers:{"Content-Type":"application/json"}});console.log("Ответ сервера:",m.data),K("Заявка успішно надіслана!"),i.reset(),A()}catch(m){console.error("Ошибка при отправке заявки:",((_=m.response)==null?void 0:_.data)||m),a(((B=(T=m.response)==null?void 0:T.data)==null?void 0:B.message)||"Сталася помилка при відправці заявки")}});async function X(){const e=new URLSearchParams({limit:30,type:"popular"}),t="https://furniture-store-v2.b.goit.study/api/furnitures";try{return(await L(t,{params:e})).data.furnitures}catch(r){return w.error({title:"Помилка",message:`Не вдалося завантажити популярні товари: ${r.message}`,position:"topRight",timeout:4e3}),[]}}const k=document.querySelector(".popular-list"),Y=async()=>{const e=await X();e.length>0?(e.forEach(t=>{k.insertAdjacentHTML("beforeend",`<li class="swiper-slide popular-item">
    ...
    `).join("");ie.feedbackList.innerHTML=t}ue();function fe(){const e=document.querySelector(".accordion-container");e&&new D(e,{duration:400,showMultiple:!1,collapse:!0,elementClass:"ac",triggerClass:"ac-trigger",panelClass:"ac-panel"})}fe();const h=document.querySelector(".filter-list");async function me(e){if(h)try{const r=`
      ...
      ${(await oe()).map(s=>`
        ...
        </li>`).join("")}`;h.innerHTML=r,h.addEventListener("click",s=>{const o=s.target.closest(".filter-btn");if(!o)return;const n=o.dataset.categoryId;e==null||e(n||void 0),h.querySelectorAll(".filter-btn").forEach(c=>c.classList.remove("active-filter")),o.classList.add("active-filter")})}catch(t){console.error(t),a("Не вдалося завантажити категорії")}}const b=document.querySelector(".card-list"),f=document.querySelector(".more-btn"),O=document.querySelector(".card-gallery .loader");let j=[],y=0;const x=8;async function ge(e){O.classList.remove("hidden");try{return(await se(1,30,e)).furnitures}catch(t){return console.error(t),a("Не вдалося завантажити меблі"),[]}finally{O.classList.add("hidden")}}async function ye(){return b?(f.style.display="none",await R(void 0),f&&f.addEventListener("click",()=>be()),{setCategory:R}):console.error("Контейнер не знайдено!")}function be(){y+=x,G()}async function R(e){y=0,b.innerHTML="",f.style.display="none";try{j=await ge(e),G()}catch(t){b.innerHTML="<p>Помилка завантаження меблів</p>",console.error(t),a("Не вдалося завантажити категорію меблів")}}function G(){const e=j.slice(y,y+x);b.insertAdjacentHTML("beforeend",e.map(t=>`
    ...
    `).join("")),f&&(f.style.display=y+x>=j.length?"none":"block")}b.addEventListener("click",async e=>{const t=e.target.closest(".card-btn");if(!t)return;const r=t.dataset.id;if(r)try{const s=await ae(r);N(s)}catch(s){console.error(s),a("Не вдалося завантажити дані меблів")}});async function ve(){const e=await ye();me(e.setCategory)}ve();
